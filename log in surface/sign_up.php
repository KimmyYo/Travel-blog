...
<?php
    ...
    elseif($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        if(explode($email, '@gmail.com') == FALSE){ //不是gmail的情況下 
            ...
            if($password != $password_check){//確認密碼不相同
                header('location:rewrite.php'); 
                
            }

            else{ //INSERT do not work
                $db = new PDO("mysql:dbname = user_info", "root", "");
                // $name = $db->quote[$name];
                // $email = $db->quote[$email];
                // $password = $db->quote[$password];

                $add_account = "INSERT INTO user_info(user_email, user_name, user_password) VALUES ($email, $name, $password)";
                $db -> exec($add_account);
